...
**沒有 AI agent、沒有網路、只有這台機器和這個 repo**。**全新 clone 缺哪些檔案**（第零節，`git clone` 之後平台不會動而錯誤訊息不會告訴你為什麼）、
四條指令、開機後必做的
一件事、**憑證怎麼自己拿到**（含進 Vault 的第 0 步：帳號、路徑、指令，不寫值）、
重點文件在哪、壞掉時的三個判斷順序，以及「平台完全不見了」的復原順序。
...
| 告警在燒 | 條件成立 | 有沒有人收到 |
| Telegram **送出成功數**在增加 | 通道活著 | 有沒有人讀 |
| Telegram `notifications_total` 在增加 | **只證明有在嘗試** | 有沒有送到——這個計數器把失敗的嘗試也算進去。2026-09-07～09-10 它一路加到 388，其中 287 次是失敗的（見 [`platform/notify/README.md`](platform/notify/README.md)） |
...
> **2026-09-10 更新——上面那句「零失敗」當時是真的，之後不是。** 同一個計數器在
> 09-07 之後累積了 287 次失敗（Telegram 拒收被截斷的 HTML），而**沒有任何地方看得
> 出來**：Prometheus 當時沒有在抓 Alertmanager，所以那個計數器沒有歷史，只能問
> 「有沒有曾經失敗過」，問不出「現在還在失敗嗎」。兩件事都已修，見
> [`platform/notify/README.md`](platform/notify/README.md)。教訓不是「要多加一個
> 指標」，是**量了一次不等於一直在量**——一次性的觀察會過期，而過期的觀察讀起來
> 和最新的一模一樣。
...
（當時實際 6,503,799；2026-09-10 已是 6,534,845，數字本身也在漂）與「31 tests」。
**兩份都連得到，兩份都是錯的。**
