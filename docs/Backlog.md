...
> **這裡的 B 編號和 `Plan.md` 主線 B 的編號不是同一份清單。** 兩邊都有 B1–B10：
> 本表的 **B10 是 Telegram token 輪替**（卡在您本人），`Plan.md` 的 **B10 是
> 流行病學週↔日曆日**（卡在疾管署）。引用時一律寫出是哪一份檔案的編號——
> `dag.py` 曾經只寫 `B10`，指向錯的那一份。2026-09-11 由 `platform/docs/xref.py`
> 的命名空間衝突檢查找出來。
...
| T26 | **急診（RODS）序列沒有進特徵集** | 它在 `rods_ed_visits` 底下、與目標不同來源，而且**完全沒有分母**（442,783 筆全空）——是次數不是率。要用就得選一個分母，那是有後果的建模決定，且這個特徵的價值尚未證明。`feature_row.er_lag_1` 欄位留著、有 COMMENT、不填值。**2026-09-15 補一條記錄裡沒有的路徑**：追 T51（`ili t+1` 三個模型家族都輸）時重讀這一段，注意到「要用就得選一個分母」預設了分母必須來自**外部**來源——而那正是 CLAUDE.md「NEVER guess or force data mappings」擋住的動作。但還有一種不需要外部分母的構造：**用這個序列自己的近期歷史把它無量綱化**，例如 `er_ratio_lag_1 = ed_count(t-1) / mean(ed_count(t-5..t-2))`，或同一地理區內的 z-score。比值沒有單位、不跨來源、不需要任何對應決定，而且保留了原本的立論（急診就診領先門診就診）。**這仍然是一個建模決定，不是缺陷修復**，而且要注意動機陷阱：`ili t+1` 現在差 6.7%，為了補那個數字去加特徵，和「搜到贏為止」是同一種錯誤——要做就要**先說清楚假說、只評估一次、輸贏都記錄**，跟 T51 裡 RidgeCV 的做法一樣 | 有人想做急診領先指標時；或流感 t+2 的區間需要再收窄時；或使用者決定把 `ili t+1` 開成獨立研究題目時（T51 的選項 A） |
...
| T38 | **`dastcov` 的覆蓋率是宣告不是量測** | `platform/security/dast_coverage.py` 的 `classify()` 用一份**寫死的四條路徑白名單**判斷「spider 找得到」，而不是讀 ZAP 實際走訪了什麼。那份清單和真正的掃描設定會分岔，而分岔的那一天報表照樣印一個看起來像量測值的數字。要改成量測需要讓 ZAP 匯出 site tree（baseline 的 JSON 只保留有警報的 URI），那是新的掃描設定 | 有人要把這個數字當覆蓋率宣稱時；或 T36 動工時一併做 |
| T39 | **兩條沒被連結的 GET 路由（`/forecast`、`/surveillance/scan`）掃不到** | 它們是無參數的 GET，掃不到只是因為沒有任何地方連結它們。最小的修法是讓 app 有一個 `/` 把自己的端點列出來（對人和 agent 的可達性也有幫助），但**在 T38 之前做沒有意義**——覆蓋率的分子是寫死的白名單，加了連結那個數字也不會動 | T38 之後 |
| T40 | **沒有任何機制保證一個控制項不是空的** | 這一輪兩次踩到：`iac` 的 fmt 夾具用 `x = 1`，而 `null_resource` 沒有那個參數，所以 validate 先失敗——那條斷言量的是它本來就有的 FAIL 分支；`test_stage_report.sh` 的夾具板是 **47 個全綠節點**，所以任何關於「誰擁有這個阻擋者」的分支在測試裡**永遠進不去**（2026-09-11 新加的所有權守衛第一版就是這樣，發現時還沒推）。三個類別裡 A（沒讀就斷言）有 `EVIDENCE_READS`＋SQL 契約，B（爆炸半徑）有 ruff F821＋精確錨點編輯，**C（空的控制項）什麼都沒有**。可能的形狀：對每個守衛跑突變測試（`capabilities/scripts/mutate`），或對測試套件量分支覆蓋率，斷言每個 refusal 分支都被進入過 | 下次有人新增一個「守衛 ＋ 控制項」的組合時；或任何一次 review 又找到一個從來沒紅過的斷言 |
| T41 | **`ubu.local` 靠 mDNS 解析，而 mDNS 今天失敗了** | 2026-09-11 `ping ubu.local` 與 `ssh` 都回 `Could not resolve hostname`，所以 `prodk8s` 與 `prodhost` 兩個節點只能回「連不上」。**這不是新缺陷，是 B7 的後果**（沒有固定 IP、沒有停用休眠），已登記為 ask `prod-node-unreachable`。這裡記的是另一件事：跨機器的驗證路徑（`run_on_ubu.sh`）**沒有任何不依賴 mDNS 的備援**，所以名稱解析一失敗，兩台機器的驗證就整條停掉，而停掉的樣子和「測試通過」在 CI 上長得不一樣但在本機很容易被當成環境問題略過 | B7 解除時一併處理；或 `run_on_ubu.sh` 要進 CI 之前 |
| T42 | **孤兒能力檢查在「剛加進來」的那一刻是瞎的** | `capability_graph.py` 用 `git ls-files` 列舉，所以**未追蹤的新檔案不在母體裡**。2026-09-11 實測：`platform/docs/xref.py` 在本機 `--check` 全綠（那時還是 untracked），commit 之後 CI 立刻把它報成孤兒。也就是說這份檢查在**作者還在現場、最容易補文件的那一刻**看不見它，要等到推出去才說話。改成 `ls-files --others --exclude-standard` 會把母體擴大到所有未忽略檔案，而其他幾份檢查都假設母體是「被追蹤的」——要一起評估，不是改一行 | 下次新增能力時；或任何一次 CI 報出本機沒報的孤兒 |
...
| T50 | **流行病學週清理把 `surveillance_fact.ingested_at` 的來源時間洗掉了** | 2026-09-14 的回滾刪掉了誤建的重複 period 與其事實，隨後 `ingest.sh` 把正確的列重新插回來——於是那些列的 `ingested_at` 全部變成 2026-09-14。實測：流感 2026w33–w36 四週的 `min(ingested_at)` 都是 `2026-09-14`，而 w33 顯然不是那天才第一次看到的。「我們什麼時候第一次看到這一週」這個問題，**現在在事實表裡問不到了**。資料本身沒錯：w35 的漂移指標用今天的庫重算得到 comparable=20、n_up=2、share 0.10，與清理前 Prometheus 記錄的值完全一致（這正是確認刪除沒有污染資料的那個測試）。錯的是**時間來源**。`ingest_runs` 還留著 `fetched_at` 與 `content_sha256`，所以答案沒有消失只是搬家——但任何把 `ingested_at` 當「首次觀測時間」讀的東西現在都會答錯。要決定：①接受並把「首次觀測」明確定義成 `ingest_runs` 的職責，②或在 fact 上加一個 upsert 不會覆寫的 `first_seen_at` | 有人要回答「這筆資料我們什麼時候拿到的」時；或 T35 的 observed provenance 要落地時（它正需要一個可信的首次觀測時間） |
| T51 | **`ili t+1` 是 MLOps 唯一擋住上線閘門的 horizon，而已註冊的兩個模型家族都輸** | 2026-09-15 用第 36 週新資料重跑既有排程 retrain（**不是改模型**，是特徵集 `seq_max` 還停在 202635，排程作業還沒看過新資料）之後：`flu t+1` **−0.08% → +0.43%**（贏了，run32 已上線）、`flu t+2` +10.79%、`ili t+2` +2.47%，**只剩 `ili t+1` −11.78%**（run30）。接著用 `backtest.py --algorithm Ridge --dry-run`（model registry 既有的第二個家族，`docs/MLOps-Model-Extension.md` 就是為此存在）在同一個 feature_set 197、同樣的 rolling-origin、同樣的基準上量：**Ridge −21.3%**（MAE 0.1431 vs 持平 0.1180，方向準確度 57.0%）——比現行的梯度提升樹更差。接著（2026-09-15 稍晚）把假說真正實作出來：原本的 `Ridge` 用 `alpha=1.0`，那是 sklearn 的預設值不是收縮決定——在 ~500 列 × ~20 欄的標準化特徵上幾乎等於沒有懲罰，所以「朝零收縮」這個假說從來沒有被測過。註冊 `RidgeCV`（alpha 由**每折訓練列內部**的 LOO-CV 選，在同一個 Pipeline 裡，rolling-origin 的保證不變），在四個 (target, horizon) **全部**跑過並寫入 model_run 34–37，不挑 horizon：`ili t+1` **−6.74%**（MAE 0.1259 vs 持平 0.1180，方向準確度 58.7%）、`ili t+2` −5.61%、`flu t+1` −10.34%、`flu t+2` −6.11%。**收縮假說在方向上被證實**（同一個 horizon：Ridge −21.3% → 樹 −11.8% → RidgeCV −6.7%，單調收斂），**但仍然輸**，而且在其他三個 horizon 輸給樹。**三個家族、四個 horizon 全部量過，我停在這裡**：再往下就是調 alpha 網格或加特徵直到那 6.7% 補上，那正是 `backtest.py` 自己寫下的反例（「that search is how a backtest becomes a slide with no predictive content」）。補充一個**不改變結論但改變解讀**的事實：replay significance 顯示 `pct_ili t+1` n=39、勝率 19/39=48.7%、sign test p=1.0000、95% CI **[−60.3%, +3.5%] 包含 0**——模型不是「可靠地更差」，是「沒有比較好」。但 `mgate` 的黃燈依設計是**事實判定**（任一 horizon 邊際 < 0 就黃，沒有任何自選顯著性門檻），**把它改成顯著性檢定就是搬球門，不要做**。MLOps 這一線只有 8 個節點，7/8=87.5%、8/8=100%，中間沒有值——所以「90% 落地」在這裡算術上等同「`ili t+1` 必須贏」 | 使用者決定要不要把 `ili t+1` 的建模開進範圍時。方向性訊號存在（57%）但幅度預測傷害大於幫助，結構上該走的是**朝零收縮**（例如在每個 fold 內做 leak-free 的 RidgeCV 選 alpha，而不是看測試結果挑），或承認 t+1 的持平基準就是這個序列的合理上限 |
| T52 | **同一支 DAST 掃描，手動跑得到 9 條路由，排程跑 ZAP 完全碰不到目標** | 2026-09-15 實測，兩次都在同一台機器、同一個目標、同一份腳本：手動 `bash platform/security/scan_dast.sh` → `scanner touched 9 distinct route(s)`；排程在 `21:10:18Z` 跑同一支 → `scanner touched 0`。而且**不是掃描器沒啟動**：`dast_seed.py` 九條全部驗證回 200 寫進 output_tail，ZAP 也回報 `sites scanned: http://host.docker.internal:18090`、`urls in alerts: 7`、`gate PASS`、`exit_code=0`、`duration 18s`。但目標容器 `21:09–21:12` 的 log 裡**一條 ZAP 的請求都沒有**（只有 9 筆 Python-urllib 與 5 筆 Prometheus）。也就是說 ZAP 產出了一份看起來正常的報告，而它宣稱掃過的那台機器沒有收到它任何一個請求。候選原因（都還沒驗）：①`mktemp -d` 在 launchd 環境下落在 Docker Desktop 沒有分享的路徑，bind-mount 靜默變空 ②排程當下 18090 上綁的是另一個容器 ③ZAP 在 spec 匯入失敗時仍會建立 site node 並對快取訊息跑被動規則。**要重現必須在排程的環境裡跑，不是在 shell 裡**——這正是它難查的原因。已先做的事：掃描在 `scanner_seen=false` 時**直接讓作業失敗**（原本 exit 0，於是 `dastcov` 隔天讀到誠實的 0%、看板變黃，卻沒有任何東西指向原因）。0% 本身是對的——覆蓋率絕不可以繼承上一次的數字——錯的是它太安靜 | 下次排程的 `dast` 作業報失敗時（現在會報了）；或有人要把其他 docker 作業掛進排程之前 |
...
| T57 | **鏡像落後時，兩個告警規則引用的指標會整個消失，而不是變成「量不到」** | 2026-09-20 實測：DuckDB 鏡像落後 5 個 ingest 批次，`platform/analytics` 依設計**拒絕回答**（正確），於是 `dataops_yoy_geo_comparable_count` 與 `dataops_yoy_geo_drift_count` 一筆都不輸出。結果是 `WidespreadGeoDrift` 這條規則**沒有東西可以比對**——它不會燒，也不會報錯，讀起來像一切正常。唯一抓到的是 test_dataops_metrics 那條「每個告警引用的指標都要真的被輸出」。候選做法：鏡像落後時仍輸出一個明確的 `dataops_yoy_unmeasured{reason="mirror-stale"} 1`，讓「量不到」是一個值而不是一個空集合 | 下次鏡像落後超過一個排程週期時；或有人要新增依賴鏡像的告警之前 |
| T58 | **Grafana 沒有 Postgres 資料源，每個 pilot 都會再遇到一次** | 2026-09-19 決定逐 origin 回測資料先寫表、再經 exporter 轉成 Prometheus 指標，因此有 156 週的匯出上限。使用者判斷這個需求會隨每個新 pilot 重現，應該做成**可重複的模式**（每 pilot 一個唯讀帳號、密碼經 Vault 注入 provisioning），而不是這次順手加一個資料源。**要先回答的是憑證輪替與曝險**，不是技術可行性 | 第二個 pilot 出現時；或有人要查超過 156 週的回測時 |
| T59 | **排程與測試還不能指定在哪台機器執行** | 2026-09-20 使用者要求「devops 要能分機器執行，但暫時沒有第二個排程宿主」。現況：排程器只跑在 Mac（launchd），`run_on_ubu.sh` 是推送前的一次性跨機器驗證，不是常態排程。ubu 現在有 prod 叢集與資料庫，所以「哪些工作該在 prod 跑」開始有實際答案（例如 prod 的備份與還原演練該由 prod 自己跑）。這會動到排程器架構，**不混在這一輪**，也不該用 cron + ssh 拼湊 | prod 承載對外服務之後；或有人要為 ubu 加第二套排程之前 |
| T60 | **能力目錄只看 git 追蹤的檔案，新腳本在提交前是隱形的** | 2026-09-20 發現：`capability_graph.py` 用 `git ls-files` 列舉，所以 `platform/dr/offsite_restore.sh`、`platform/db/pilot_db.sh`、`platform/k8s/prod-vault/bringup.sh` 這三支在提交前跑檢查都是「零孤兒」——**而它們當時根本不在清單裡**。這不是誤判，是「沒被問到」：一個看起來全綠的檢查，涵蓋範圍卻不含剛寫好的東西。候選做法：檢查時把未追蹤但可執行的檔案列成 `UNTRACKED`（不是 orphan，但也不能當成不存在）| 下次有人依賴「零孤兒」來判斷新能力有沒有被描述時 |
| T61 | **量測式週期需要「一次抓取裡的多個資源不是多次發布」這個概念，目前用日界近似** | 2026-09-23：`moi-ris-village-population` 一次抓取連續取回多個年度資源，每份內容不同，於是被量成「8 秒發布一次」，門檻變成 24 秒、永遠燒。已用三道修正擋住：變更先收斂到**不同日期**、低於一天的量測值一律退回宣告值（`CADENCE_FLOOR_SECONDS`）、`structural` 證據優先於量測（介面一年一份，統計估計贏不過它）。**剩下的近似**：同一天內的多次真實發布會被併成一次，而跨午夜的單次抓取會被算成兩天。正解是把 `ingest_runs` 的 run id 帶進來，以「同一次執行」為界而不是以日期為界——需要 ingest 端也記 run id | 有來源真的一天發布多次時；或 ingest 端加上 run id 之後 |
| T62 | **`fcscore`（已發布預測的事後評分）目前歸類為「工程自理」，但工程端關不掉它** | 2026-09-23 板面：`fcscore` 轉黃，理由是「已發布預測事後評分：2/5 勝過持平基準（n=5，尚不足以下結論）」。沒有對應的 ask，所以它落在 `eng` 預設，MLOps 因此從 87.5% 掉到 75%。但「發出去的預測準不準」是模型事實，工程端能做的只有把它量出來——而那已經做了。這和 `mgate` 是同一類（research），差別只在 mgate 有一筆具名的 ask。**我沒有自己改歸類**：把項目從 `eng` 移到 `research` 會**提高工程端上限**，那種讓數字變好看的改動要由你決定（ADR-0021 的前例：prod-workload-reachability 是先量測再改歸類）。要做的話是補一筆 ask，說明 n 還太小、以及要多少樣本才算結論 | 你決定 fcscore 該歸誰時；或 n 大到足以下結論時 |
