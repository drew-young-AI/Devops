...
**落地是一個三態判定，逐線計算（ADR-0021）。** 它由 `stage_report.py` 算出來，
寫在每一條線的 `completion.verdict`，不是每輪用散文重新爭論一次：

| 判定 | 條件 | 意思 |
|---|---|---|
| `LANDED` | `工程自理` = 0 **且** `pct_strict` ≥ 90% | 沒有東西卡在我們手上，平台也夠綠 |
| `ENG-DEBT` | `工程自理` > 0 | **還有東西卡在我們手上**，不管百分比多高 |
| `CEILING-BOUND` | `工程自理` = 0 **且** `pct_strict` < 90%（此時必然 = `pct_ceiling_eng_only`） | 工程已經做到頂，剩下的不是工程問題 |

兩個問題以前被塞進同一個數字裡，現在分開問：

| | 條件 | 它答什麼 |
|---|---|---|
| 1 | `pct_strict` **≥ 90%** | **平台夠不夠綠**。分母是**節點**不是階段——同一個平台用階段算是 33%、用節點算是 84%，讀的人分不出自己在看哪一個 |
| 2 | 阻擋者裡 **`工程自理` = 0** | **還有沒有事情卡在我們自己手上**。百分比會被分母影響，這一條不會 |

**只有第 1 條會被「加節點」影響，第 2 條不會——所以兩條都要。**
而且 `ENG-DEBT` 是**先判的**：93.3% 而還有一個工程阻擋者，讀 `ENG-DEBT`
不讀 `LANDED`——這比舊規則**更嚴**，不是更鬆。

`CEILING-BOUND` **不是 `LANDED` 的別名**。它不主張這條線夠好，只主張一件窄而
可驗證的事：**所有非綠節點都已經由一筆具名 ask 指派給 decision／research／
external**。百分比一分都不會因為這個判定改變。

### 先看 `pct_ceiling_eng_only`，它會告訴你這一輪值不值得投工程

那一欄是「**把所有 `工程自理` 的阻擋者都關掉、其他都不變**」會得到的百分比。

```bash
python3 -c "import json;d=json.load(open('docs/Stage-Report.json'));\
[print(l['name'], l['completion']['pct_strict'], '->', l['completion']['pct_ceiling_eng_only']) for l in d['lines']]"
```

**它低於 90% 的那一刻，代表這個 repo 裡再多的工作也到不了標準。**
那是關於標準的事實，不是關於努力的事實。2026-09-10 三條線分別是
86.2%／88.9%／87.5%——**三條都低於 90%**，所以那一天之後所有的路都經過
使用者的決定、外部單位、或一個誠實的研究結果。

這個欄位存在的理由很實際：同一段話原本每一輪都要重寫一次，而**沒有人需要
重新推導的數字，就沒有人會推導錯**。
