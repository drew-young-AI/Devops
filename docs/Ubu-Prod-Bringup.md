...
**mDNS 也可能只回 IPv6（2026-09-10 實測到）。** `dscacheutil -q host -a name
ubu.local` 只吐 `ipv6_address:`、沒有 A 記錄，而 ssh 偏好 IPv6，於是連到一個
從這個網段路由不到的全域位址。**症狀是逾時或「無法解析主機名」，兩者都讀起來
像機器關著**——而那天機器開著、22 埠開著、`nc -z 192.168.1.143 22` 立刻成功。
這個誤診發生過兩次。修法是 `ssh -4`；`platform/tests/run_on_ubu.sh` 的每一個
ssh 與 rsync 都已加上。
...
註：**IP 會變，不要記住它。** 這份手冊先寫 `.143`，改成 `.144`，
2026-09-09 又量到 `.143`。這不是筆誤來回，是 DHCP 沒有保留位址（B7，只有
使用者能做）。診斷時用 `arp -a` 與整段 ping sweep 找它，不要用記在文件裡的
數字——**一個會過期的事實寫在手冊裡，比不寫更糟，因為它讓人停止查證**。
2026-09-09 的一次診斷就因此白走：手冊說 `.144`，掃描確認整個網段沒有任何
主機開著 22 或 6443，結論「ubu 不在這個網段」是對的（它當時真的還沒開機），
但如果它當時在 `.143` 開著，那個結論會是錯的而且看起來一樣有根據。

**ubu 上沒有 Docker。** 它跑 k3s over containerd。`platform/tests/run_on_ubu.sh`
的 tier 1 因此不能包含任何需要容器映像的套件——`test_model_registry.sh` 曾被
宣告成 tier 1（「無依賴」），而它的每一條斷言都在 pilot 映像裡跑。在這台機器
和 GitHub runner 上 Docker 永遠都在，所以那個錯誤的宣告看不見。已改成
SKIP ＋ UNVERIFIED。
