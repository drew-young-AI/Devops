...
八張圖不是同一個對象。**不要從圖 01 開始看**，它是三條線裡節點最多的一張。

| 順序 | 圖 | 它回答什麼 | 給誰 |
|---|---|---|---|
| 1 | **圖 07 大統整流程圖** | 三條線怎麼接成一條鏈，兩個交界各是什麼條件 | 長官。**只給一張的話就是這張** |
| 2 | 圖 08 大統整架構圖 | 由下往上五層：機器、共用基礎、三條線、觀測、人打開的那一層 | 長官 |
| 3 | 圖 03／04 DataOps | 資料從哪裡來、存在哪裡、哪一步是後面的前提 | 長官、資料負責人 |
| 4 | 圖 05／06 MLOps | 一次重訓要通過什麼才能變成對外預測，沒通過會怎樣 | 長官、公衛預測 |
| 5 | 圖 01／02 DevOps | 這條線由哪些東西組成、一次變更要過哪些關卡 | **工程附錄**：節點多、名字是行話（SAST、DAST、k3d、Vault） |

每張圖下面的「**要看的一件事**」是那張圖的重點，讀那一段就夠。
圖上**沒有數字也沒有燈號**——數字一週就過期，現況一律看
[階段報告](http://mac.local:18085/Stage-Report.html)。
...
| 缺陷 | 症狀 | 真正原因 |
|---|---|---|
| 圖與板面分岔 | 圖 02 仍畫著 09-10 已退役的 CI、Trivy、Registry、production-like；18 個現行節點沒出現在任何一張圖上；橫幅寫著 09-01 的數字 | 圖頭寫著「所有節點取自 dag.py」，**沒有任何東西檢查這句話**。現在由上一節的投影檢查守著，數字則整個移出圖外（改指向階段報告） |
| 第二個渲染器 | 離線版帶著 `claude-mermaid-runtime` 區塊，會並行渲染全部圖（就是上表第二個缺陷）並套主機的米色配色 | Artifact 主機會在它提供的頁面注入自己的 runtime；09-02 把發布後的頁面讀回來存成來源，連同注入的區塊一起進了 git，`build.sh` 再照抄進離線版。**讀回發布頁時，不要整份存回來源** |

**mermaid 排版的一個經驗規則**（同日實測，靠量字級找到的）：subgraph 內的節點只要跟外面的節點有連線，
該 subgraph 的 `direction` 就會被忽略，整串攤成一列——圖 01、08 因此縮到 3–5px。
改法是跨群組只留必要的相依邊，其餘排版用 `~~~` 控制階層。

## 測試能證明什麼、不能證明什麼

`test_offline_report.sh`（tier 1，24 項）證明：
...
- **圖仍是 `dag.py` 的投影**：每個板面節點都畫在某張圖上、標籤與板面一字不差；
  每條相依邊都畫出來；沒有已退役的名字；兩個檢查點之間沒有 `dag.py` 不存在的箭頭。
  附三個突變控制項，且每個突變必須真的改到檔案（否則控制項本身失敗）
- 來源與離線版裡沒有 Artifact 主機注入的 mermaid runtime（見下一節第四個缺陷）

**讓「投影」可以被機器檢查的慣例**：mermaid 節點 id **小寫開頭＝`dag.py` 節點 id**，
標籤第一行必須等於板面標籤，`<br/>` 之後可以加給人看的白話說明；**大寫開頭＝脈絡**
（資料庫、機器、讀者），不受檢查。排版用的不可見連線 `~~~` 不算相依邊。

**它不能證明圖畫得出來、字讀得清楚。** 那需要瀏覽器。2026-09-02 是人工用真的 Chrome 驗的；
2026-09-17 改用本機 Chrome headless（不需安裝任何東西）：`--dump-dom` 確認八個 SVG 都出來、
無 mermaid 語法錯誤，再用 SVG `viewBox` 寬度對版面寬度（約 1030px）換算**實際字級**，
低於約 11px 的圖截圖目視。這一步仍是人工，明講，不假裝測試涵蓋了它。
