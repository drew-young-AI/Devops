...
目前平台完成到：local CI + local observability + Git-triggered CI（GitHub Actions）+ IaC skeleton（OpenTofu，provider-neutral contract）+ local HTTPS/NGINX adapter。Deployment adapter（develop/production-like promotion）與 Public URL 尚未完成，詳見根目錄 `Plan.md` 的 handoff status。
...
├── compose/         # 平台元件的 Compose adapter（尚未建立）
