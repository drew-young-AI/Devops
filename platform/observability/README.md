...
| [`scripts/verify_dashboard_render.py`](scripts/verify_dashboard_render.py) | 隨 `test_dashboards.sh` 每次跑；改看板或改 datasource 之後手動跑 | 把每一張看板的每一個面板查詢**送過 Grafana 自己的 datasource proxy**，回報幾個真的拿到序列 | 它證的是 `dashboard_audit.py` 證不到的那半：憑證通得過、uid 解析得到、Grafana 的容器連得到 Prometheus／Loki。靜態稽核在一台正在拒絕每一次登入的 Grafana 上會通過。憑證讀自 `.grafana.env`，不印出任何內容；Prometheus 與 Loki 走不同端點，log query 必須用 `query_range` |
